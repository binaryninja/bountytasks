...
TARGET_HOST="${TARGET_HOST:-gramps-app:5000}"
...
if echo "$OUTPUT" | grep -qi "success\|traversal\|created\|written\|slip"; then
    echo "Verification Passed: Zip Slip path traversal exploit succeeded"
    exit 0
fi
